...
public class Car3 : MonoBehaviour
{
	...
	void Start()
	{
		...
		GetComponent<Rigidbody>().centerOfMass = cof.localPosition;
	...
	void WheelPosition()
	{
		...
		wheelFLTrans.position = position;
		wheelFLTrans.rotation = rotation;

		wheelFR.GetWorldPose(out position, out rotation);
		wheelFRTrans.position = position;
		wheelFRTrans.rotation = rotation * Quaternion.Euler(0, 180, 0);

		wheelRL.GetWorldPose(out position, out rotation);
		wheelRLTrans.position = position;
		wheelRLTrans.rotation = rotation;

		wheelRR.GetWorldPose(out position, out rotation);
		wheelRRTrans.position = position;
		wheelRRTrans.rotation = rotation * Quaternion.Euler(0, 180, 0);
	...
	void CalcDownForceOnCar()
	{
		...
		{
			//hit.point is the point the raycast is hitting
			GetComponent<Rigidbody>().AddForce(0, carSpeed * 1000 * -1, 0);
			//print ("1000");
		...
		{//car is in air
			int force = 175;
			GetComponent<Rigidbody>().AddForce(0, carSpeed * force * -1, 0);
			if (!ejump)
			{
				GetComponent<Rigidbody>().AddForceAtPosition(new Vector3(0, carSpeed * 6, 0), forceUp.position);
			}
			//print (force);
	...
	void SingleJump()
	{
		if (forSingleJump)
		{
			if (!jumped)
			{
				...
				{
					//print (canJumpTimer);
					GetComponent<Rigidbody>().AddForce(Vector3.up * 1000000);
					jumped = true;
					ejump = true;
				}
			}
	...
	void CarBodyMove()
	{
		...
		if (carSpeed != 0)
		{
			...
			{//make this for slow speeds and increase 0.25 to about 0.35 for speeds larger thatn 70-80
	...
	void FixedUpdate()
	{
		...
		if (airplaneMode && !onGround)
		{
			...
			GetComponent<Rigidbody>().AddRelativeForce(Vector3.forward * Input.GetAxisRaw("Horizontal") * 150 * carSpeed);
			print(GetComponent<Rigidbody>().velocity.magnitude + " and " + GetComponent<Rigidbody>().angularVelocity.magnitude);
		}
